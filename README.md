...
> 如需把签到结果推送到飞书群机器人，在 `config.json` 填入 `feishuWebhook`（或用环境变量 `WORKBUDDY_FEISHU_WEBHOOK`）；推送由 `signin.py` 内部完成，不再依赖 `.bat` 里的硬编码。
...
配置文件 `config.json`（项目根目录，已被 `.gitignore` 忽略，**请勿提交**）支持以下字段；也可通过环境变量覆盖，**优先级：环境变量 > config.json > 内置默认**。

| 字段 / 环境变量 | 作用 | 默认 |
|---|---|---|
| `endpoint` / `WORKBUDDY_ENDPOINT` | 签到接口 endpoint | `https://copilot.tencent.com` |
| `authFile` / `WORKBUDDY_AUTH_FILE` | 自动探测失败时手动指定凭据文件路径 | 自动探测 |
| `feishuWebhook` / `WORKBUDDY_FEISHU_WEBHOOK` | 飞书群机器人 webhook，配置后自动推送签到结果 | 不推送 |

> 模板见 `config.example.json`：复制为 `config.json` 并填入你的 webhook 即可。所有推送走 `config.json`/环境变量，不再硬编码于脚本或 `.bat`，便于安全分享、也避免密钥进版本库。
...
├── scripts/
│   └── notify.ps1            # (可选) 独立飞书推送工具：读取日志文件推送到 webhook；signin.py 现已内置推送
├── README.md
├── LICENSE
└── CHANGELOG.md
```
